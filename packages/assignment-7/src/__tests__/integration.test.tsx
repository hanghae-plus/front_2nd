...
describe('일정 관리 애플리케이션 통합 테스트', () => {
  ...
  test('초기 일정 목록이 올바르게 렌더링 된다', async () => {
    render(<App />);

    expect(await screen.findAllByText('팀 회의')).toBeInTheDocument();
    expect(await screen.findAllByText('점심 약속')).toBeInTheDocument();
    expect(await screen.findAllByText('프로젝트 마감')).toBeInTheDocument();
    expect(await screen.findAllByText('생일 파티')).toBeInTheDocument();
    expect(await screen.findAllByText('운동')).toBeInTheDocument();
    expect(await screen.findAllByText('알림 테스트')).toBeInTheDocument();
  });
