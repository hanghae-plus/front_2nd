...
//이벤트 핸들러
const addItemHandler = (e, updateCart) => {
  ...
  if (i) {
    ...
    //선택한 상품이 장바구니에 없다면??
      ...
      const ct = document.querySelector('#cart-items');
      ct.appendChild(d);
    }
    //금액 업데이트
    updateCart();
...
function main() {
  ...
  function updateCart() {
    ...
    const { newDiscountRate, newTotal } = caculateOptimalDiscount(
      totalQuantity,
      totalBeforeDiscount,
      total
    );

    const discountRateRounded = Math.round(newTotal);

    const $total = document.querySelector('#cart-total');
    $total.textContent = `총액: ${discountRateRounded}원`;
  ...
  addButton.addEventListener('click', (e) => addItemHandler(e, updateCart));
