// createHooks 함수는 callback을 인자로 받아 내부 훅 상태를 관리하는 훅 생성 함수를 반환
export function createHooks(callback) {
  // context 배열은 훅의 상태 값을 저장
  let context = [];
  // index는 현재 훅의 위치를 추적
  let index = 0;
  // isPending는 렌더링이 예약되었는지를 추적
  let isPending = false;
  // nextRenderCallback은 다음 렌더링에 실행할 콜백 함수를 저장
  let nextRenderCallback = null;

  // scheduleRender 함수는 렌더링을 예약
  const scheduleRender = () => {
    // 렌더링이 이미 예약되지 않았으면 예약
    if (!isPending) {
      isPending = true;
      // 다음 프레임에서 콜백을 실행
      requestAnimationFrame(() => {
        isPending = false;
        // nextRenderCallback이 있으면 실행
  ...
  const useState = (initState) => {
    ...
    const setState = (newState) => {
      ...
      if (context[currentIndex] !== newState) {
        ...
        // context를 리셋하고 nextRenderCallback을 설정
        resetContext();
