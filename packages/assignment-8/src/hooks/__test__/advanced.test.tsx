...
describe("일정 관리 애플리케이션 통합 테스트", () => {
  describe("일정 CRUD 및 기본 기능", () => {
    test("새로운 일정을 생성하고 모든 필드가 정확히 저장되는지 확인한다", async () => {
      ...
      // 일정 정보 입력
      await user.type(screen.getByLabelText("제목"), "인스턴트 회의");
      await user.type(screen.getByLabelText("날짜"), "2024-08-20");
      ...
      expect(eventList).toHaveTextContent("인스턴트 회의");
      expect(eventList).toHaveTextContent("2024-08-20");
  ...
  describe("반복 일정", () => {
    test("월말에 반복되는 일정을 생성하고 올바르게 표시되는지 확인한다", async () => {
      const { user } = setup(<App />);

      // 1. 일정 추가 폼의 모든 필드를 채웁니다.
      await user.type(screen.getByLabelText("제목"), "월말 회의");
      await user.type(screen.getByLabelText("날짜"), "2024-08-31");
      await user.type(screen.getByLabelText("시작 시간"), "14:00");
      await user.type(screen.getByLabelText("종료 시간"), "15:00");
      await user.type(screen.getByLabelText("설명"), "월말 정기 회의");
      ...
      // 3. 8월부터 12월까지 각 월의 말일에 일정이 표시되는지 확인
      ...
      for (const month of months) {
        ...
        expect(
          within(monthView).getByText(`2024년 ${month}`)
        ).toBeInTheDocument();

        // 월의 마지막 날짜 셀을 찾습니다.
        const lastDayCell = within(monthView).getAllByRole("cell").pop();
        expect(within(lastDayCell!).getByText("월말 회의")).toBeInTheDocument();
      }

      // 4. 이벤트 목록에서 반복 일정 정보 확인
      const eventList = screen.getByTestId("event-list");
      const eventItems = within(eventList).getAllByText("월말 회의");
      expect(eventItems).toHaveLength(5); // 8월부터 12월까지 5개의 일정

      const firstEvent = eventItems[0].closest("div");
      expect(
        within(firstEvent!).getByText("반복: 1월마다 (종료: 2024-12-31)")
      ).toBeInTheDocument();
      expect(
        within(firstEvent!).getByText("알림: 1시간 전")
      ).toBeInTheDocument();
    });
  });
  ...
  describe("일정 뷰 및 필터링", () => {
    ...
    test("월별 뷰에 일정이 정확히 표시되는지 확인한다", async () => {
      ...
      const monthEventTitleEl =
        await within(monthView).findByText("이번달 팀 회의");
  ...
  describe("검색 기능", () => {
    ...
    test("제목으로 일정을 검색하고 정확한 결과가 반환되는지 확인한다", async () => {
      ...
      expect(
        within(eventList).getByText("2024-08-01 09:00 - 10:00")
      ).toBeInTheDocument();
      expect(
  ...
  describe("일정 충돌 감지", () => {
    test("겹치는 시간에 새 일정을 추가할 때 경고가 표시되는지 확인한다", async () => {
      ...
      expect(
        screen.getByText("기존 회의 (2024-08-01 09:00-10:00)")
      ).toBeInTheDocument();
    });
    ...
    test("기존 일정의 시간을 수정하여 충돌이 발생할 때 경고가 표시되는지 확인한다", async () => {
      ...
      expect(screen.getByText(/다음 일정과 겹칩니다/)).toBeInTheDocument();
      expect(
        screen.getByText("다른 회의 (2024-08-01 08:00-09:00)")
      ).toBeInTheDocument();
    });
  });
