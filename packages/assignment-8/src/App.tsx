...
function App() {
  ...
  const addOrUpdateEvent = async () => {
    ...
    if (isRepeating) {
      console.log(isRepeating, eventData.repeat, repeatType);
      // 반복 항목인 경우 중복 검사를 하지 않음
  ...
  const saveRepeatEvent = async (eventData: Event) => {
    const repeatEventDate = new Date(`${eventData.date}T00:00:00`);

    while (
      repeatEventDate < new Date(eventData.repeat.endDate ?? "2024-12-31")
    ) {
      const currentEvent = new Date(repeatEventDate);
      const currentEventYear = currentEvent.getFullYear();
      const currentEventMonth = currentEvent.getMonth();
      const currentEventDate = currentEvent.getDate();

      const repeatEvent = {
        ...eventData,
        id: editingEvent ? editingEvent.id : Date.now(),
        date: `${currentEventYear}-${currentEventMonth + 1}-${currentEventDate}`,
      };
      await saveEvent(repeatEvent);

      switch (eventData.repeat.type) {
        case "daily":
          repeatEventDate.setDate(currentEventDate + eventData.repeat.interval);
          break;
        case "weekly":
          repeatEventDate.setDate(
            currentEventDate + eventData.repeat.interval * 7,
          );
          break;
        case "monthly":
          repeatEventDate.setMonth(
            currentEventMonth + eventData.repeat.interval,
          );
          break;
        case "yearly":
          repeatEventDate.setFullYear(
            currentEventYear + eventData.repeat.interval,
          );
          break;
      }

      console.log(repeatEventDate);
    }
  };
  ...
  const formatWeek = (date: Date): string => {
    ...
    if (currentDay < startDay) weekNumber++;
        ...
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <Button
                data-testid="duplicated-continue-button"
                colorScheme="red"
                onClick={() => {
