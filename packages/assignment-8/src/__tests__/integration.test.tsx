...
describe('일정 관리 애플리케이션 통합 테스트', () => {
  ...
  describe('일정 뷰 및 필터링', () => {
    ...
    test('날짜를 이동할 수 있다. > 일정 뷰 모드가 month일 때, 월 변경', async () => {
      ...
      const initialMonthView = await waitFor(() =>
        screen.getByTestId('month-view')
      );
      expect(initialMonthView).toBeInTheDocument();

      // 초기 날짜 확인 (2024년 7월)
      expect(initialMonthView).toHaveTextContent('2024년 7월');

      // 이전 달로 이동
      const prevButton = screen.getByLabelText('Previous');
      await user.click(prevButton);

      const prevMonthView = await waitFor(() =>
        screen.getByTestId('month-view')
      );

      await waitFor(() => {
        expect(prevMonthView).toHaveTextContent('2024년 6월');
      });
      ...
      const nextMonthView = await waitFor(() =>
        screen.getByTestId('month-view')
      );

      await waitFor(() => {
        expect(nextMonthView).toHaveTextContent('2024년 8월');
      });
    });
    ...
    test('날짜를 이동할 수 있다. > 일정 뷰 모드가 week 때, 주 변경', async () => {
      ...
      // 뷰 상태 확인 (week view)
      const initialWeekView = await waitFor(() =>
        screen.getByTestId('week-view')
      );
      expect(initialWeekView).toBeInTheDocument();

      // 초기 날짜 확인 (2024년 7월 1주 )
      expect(initialWeekView).toHaveTextContent('2024년 7월 1주');

      // 이전 주로 이동
      const prevButton = screen.getByLabelText('Previous');
      await user.click(prevButton);

      const prevWeekView = await waitFor(() => screen.getByTestId('week-view'));

      await waitFor(() => {
        expect(prevWeekView).toHaveTextContent('2024년 6월 4주');
      });
      ...
      const nextWeekView = await waitFor(() => screen.getByTestId('week-view'));

      await waitFor(() => {
        expect(nextWeekView).toHaveTextContent('2024년 7월 2주');
      });
    });
    ...
    test('월별 뷰에 일정이 정확히 표시되는지 확인한다', async () => {
      ...
      // 캘린더 셀 확인
      const cell20 = within(monthView).getByTestId('month-cell-7-20');
      const cell21 = within(monthView).getByTestId('month-cell-7-21');
      const cell22 = within(monthView).getByTestId('month-cell-7-22');
      const cell25 = within(monthView).getByTestId('month-cell-7-25');
      const cell28 = within(monthView).getByTestId('month-cell-7-28');
  ...
  describe.skip('알림 기능', () => {
    beforeEach(() => {
      ...
      // 이벤트 시작 시간 10분 전의 시간을 계산
      const [hours, minutes] = event6.startTime.split(':').map(Number);
      ...
      const tenMinutesBefore = `${event6.date}T${String(newHours).padStart(2, '0')}:${String(newMinutes).padStart(2, '0')}`;
    ...
    test('일정 알림을 설정하고 지정된 시간에 알림이 발생하는지 확인한다', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen).toHaveTextContent('알림 테스트');
      });
    });
  });
  ...
  describe('검색 기능', () => {
    test('제목으로 일정을 검색하고 정확한 결과가 반환되는지 확인한다', async () => {
      ...
      await waitFor(() => {
        ...
        // 다른 일정들은 보이지 않아야 합니다.
        expect(eventItem).not.toHaveTextContent('점심 약속');
        expect(eventItem).not.toHaveTextContent('프로젝트 마감');
        expect(eventItem).not.toHaveTextContent('생일 파티');
        expect(eventItem).not.toHaveTextContent('운동');
      });
    });
  ...
  describe('공휴일 표시', () => {
    test('달력에 1월 1일(신정)이 공휴일로 표시되는지 확인한다', async () => {
      ...
      const monthView = await screen.findByTestId('month-view');
      const currentCell = within(monthView).getByTestId('month-cell-1-1');

      // '신정'이라는 글자가 있는지 확인
      const holidayText = within(currentCell).getByText('신정');
    ...
    test('달력에 5월 5일(어린이날)이 공휴일로 표시되는지 확인한다', async () => {
      ...
      await waitFor(async () => {
        ...
        const monthView = await screen.findByTestId('month-view');
        const currentCell = within(monthView).getByTestId('month-cell-5-5');

        // '어린이날'이라는 글자가 있는지 확인
        const holidayText = within(currentCell).getByText('어린이날');
        expect(holidayText).toBeInTheDocument();
      });
    });
  });
