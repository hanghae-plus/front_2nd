  ...
  describe("반복 일정", () => {
    ...
    describe("반복 달력 내 확인", () => {
      test("일정 생성 시, 반복 일정을 달력에서 확인할 수 있다", async () => {
        ...
        await waitFor(() => {
          const eventCells = screen
            .getAllByRole("cell")
            .filter(
              (cell) =>
                Number(cell.textContent) >= 15 && Number(cell.textContent) <= 31
            );

          const noEventCells = screen
            .getAllByRole("cell")
            .filter((cell) => Number(cell.textContent) < 15);

          const testCells = screen.getAllByRole("cell");

          testCells.forEach((value) => console.log(value.innerHTML));

          // title 있는지 확인
          for (const cell of eventCells) {
            const eventBoxes = within(cell).queryAllByTestId("event-box");

            console.log(cell.innerHTML);

            const hasEventWithTitle = eventBoxes.some((box) =>
              box.textContent?.includes("테스트 일정 반복 달력 확인")
            );
      ...
      test("일정 수정 시, 반복 일정을 달력에서 확인할 수 있다", async () => {
        ...
        const repeatInterval = screen.getByLabelText("반복 간격");
        await userEvent.clear(repeatInterval);
        await userEvent.type(repeatInterval, "11");
        ...
        const cells = screen
          ...
          .filter(
            (cell) =>
              Number(cell.textContent) >= 15 && Number(cell.textContent) <= 24
          );
        // title 있는지 확인
        for (const cell of cells) {
          await expect(cell).toHaveTextContent("테스트 일정 반복 달력 확인");
        }
      });
    });
  });
