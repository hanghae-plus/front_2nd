...
type countType = {crying: number}

// NOTE: 다른 파일은 수정하지 않고, 현재 파일만 수정하여 문제를 해결해주세요.
const CatRender = memo(({crying}: countType) => <Cat crying={crying}/>);
const DogRender = memo(({crying}: countType) => <Dog crying={crying}/>);
...
export default function PureComponentTest() {
  ...
  return (
    <div>
      <CatRender crying={meowCount}/>
      <DogRender crying={barkedCount}/>
