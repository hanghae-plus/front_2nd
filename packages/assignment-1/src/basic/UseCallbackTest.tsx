...
export default function UseCallbackTest() {
  ...
  const incrementMeow = useCallback(() => setMeowCount((n) => n + 1), []);
  const incrementBarked = useCallback(() => setBarkedCount((n) => n + 1), []);

  return (
    <div>
      <p data-testid="cat">meowCount {meowCount}</p>
      <p data-testid="dog">barkedCount {barkedCount}</p>
      <MeowButton onClick={incrementMeow} />
      <BarkButton onClick={incrementBarked} />
    </div>
  );
}
