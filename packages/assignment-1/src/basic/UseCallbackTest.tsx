...
export default function UseCallbackTest() {
  ...
  const clickMeowHandler = useCallback(() => setMeowCount((n) => n + 1), []);
  const clickBarkHandler = useCallback(() => setBarkedCount((n) => n + 1), []);

  return (
    <div>
      <p data-testid="cat">meowCount {meowCount}</p>
      <p data-testid="dog">barkedCount {barkedCount}</p>
      <MeowButton onClick={clickMeowHandler} />
      <BarkButton onClick={clickBarkHandler} />
    </div>
  );
}
