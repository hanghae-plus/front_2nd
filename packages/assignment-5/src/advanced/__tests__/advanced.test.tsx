...
import { calculateCartTotal } from "../../refactoring/hooks/utils/cartUtils";
...
describe("advanced > ", () => {
  ...
  describe("자유롭게 작성해보세요.", () => {
    ...
    test("비율 할인이 들어간 장바구니 계싼", () => {
      ...
      expect(result).toEqual({
        totalBeforeDiscount: 600,
        totalAfterDiscount: 513,
        totalDiscount: 87,
      });
    });

    test("localStorage에서 기존 값을 읽어오기", () => {
    ...
    test("값이 업데이트되면 localStorage에 저장", () => {
      const { result } = renderHook(() =>
        useLocalStorage("testKey", "defaultValue")
      );
      act(() => {
