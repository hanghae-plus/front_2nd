...
import { calculateDiscountedPrice } from "../../refactoring/hooks/utils/cartUtils";
...
describe("advanced > ", () => {
  ...
  describe("자유롭게 작성해보세요.", () => {
    ...
    test("새로운 hook 함수를 만든 후에 테스트 코드를 작성해서 실행해보세요", () => {
      const { result } = renderHook(() => useDiscountCalculator(10000));

      expect(result.current.price).toBe(10000);
      expect(result.current.discount).toBe(0);
      expect(result.current.calculateFinalPrice()).toBe(10000);

      act(() => {
        result.current.setDiscount(0.2);
      });

      expect(result.current.discount).toBe(0.2);
      expect(result.current.calculateFinalPrice()).toBe(8000);

      act(() => {
        result.current.setPrice(20000);
      });

      expect(result.current.price).toBe(20000);
      expect(result.current.calculateFinalPrice()).toBe(16000);
    });
  });
});
